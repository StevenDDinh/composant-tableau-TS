...
declare const Tabulator: typeof import("tabulator-tables").TabulatorFull;

// Création d'un nouveau type de filtre de date sur une période définie
Tabulator.extendModule?.("filter", "filters", {
    "dateRange": function(headerValue:any, rowValue: any) { 
        if (!rowValue || !headerValue.dateDeb || !headerValue.dateFin) return true;
...
const recupDate: (T: string, U: string) => Date[] = (idDateDeb:string, idDateFin:string)=>{
    ...
    const dateDebNew: Date = new Date(dateDeb);
    const dateFinNew: Date = new Date(dateFin);

    return [dateDebNew, dateFinNew]
}

// Fenêtre de filtre par colonne générique
const filtreGeneral: (event:Event, column: any) => HTMLDivElement = ( event,column) => {
    // Conteneur principal
    const container = document.createElement("div");
    container.className = "popup-filtre";

    // Récupération des données afin de les affichés dans les listes de selection
    const field:string = column.getField(); // Récupère le nom de la colonne
    const allData:any[] = column.getTable().getData(); // Récupère toutes les lignes de la colonne
    ...
    const checkboxes:HTMLInputElement[] = []; // Tableau pour stocker les cases à cocher
    const labelElements:{ el: HTMLLabelElement, val: string }[] = []; // tableau pour stocker les labels à filtrer
    ...
    selectAllCb.addEventListener("change", (event) => {
        checkboxes.forEach(cb => cb.checked = (<HTMLInputElement>event.target).checked );
    });
    ...
    searchInput.addEventListener("input", (event) => {
        const valeurRecherche = (<HTMLInputElement>event.target).value.toLowerCase();
...
const filtreDate: (event: any, column: any) => HTMLDivElement  = (event, column)=>{
    // Conteneur principal
    const container = document.createElement("div");
    container.className = "popup-filtre";
    ...
    btnValider.addEventListener("click",()=>{
        ...
        const listeDate:Date[] = recupDate(".popup-input-date-deb",".popup-input-date-fin");
...
const table = new Tabulator("#id-table", {
    ajaxURL: "./data.json", //Chargement des données json
    ...
    paginationCounter: (totalRows:any)=>{
        return `  ${totalRows} résultats`;
    }, // Compte le nombre de ligne totals
    paginationSizeSelector:[5, 10, 20], // Liste de nombre de lignes à afficher

    rowHeight:45, // Hauteur de chaque ligne

    // Case à cocher
    rowHeader: {
        field: "selection",
        headerSort: false, // Désactive le trie
        frozen:true, // Permet de bloquer la colonne
...
// Partie evenement
const barrRecherche:HTMLInputElement|null = document.querySelector("#input-barre-rechercher");

const btnFiltreGlobal:HTMLButtonElement|null = document.querySelector("#btn-filtre");
const fntFiltre:HTMLElement|null = document.querySelector("#popup-interrogation");
const btnFermer:HTMLButtonElement|null = document.querySelector("#fermer-interrogation");
const btnAnnuler:HTMLButtonElement|null = document.querySelector(".btn-annuler-av");
const btnValider:HTMLButtonElement|null = document.querySelector(".btn-valider-av");
const btnReinitialiser:HTMLButtonElement|null = document.querySelector(".btn-reinitialiser-av");

const selectNom:HTMLSelectElement|null = document.querySelector("#nomSelect");
const selectPrenom:HTMLSelectElement|null= document.querySelector("#prenomSelect");
const selectFonction:HTMLSelectElement|null = document.querySelector("#fonctionSelect");

const inputDateDeb:HTMLInputElement|null = document.querySelector("#dateDeb-global");
const inputDateFin:HTMLInputElement|null = document.querySelector("#dateFin-global");

//Barre de recherche
barrRecherche?.addEventListener("keyup",()=>{
    const valeur:string = barrRecherche.value;
...
document.addEventListener("DOMContentLoaded", () => {  // Attend que tout le script HTML soit chargé
    const btnOrganiser:HTMLButtonElement|null = document.querySelector("#btn-organiser-colonne");
    const popupOrganisation:HTMLElement|null = document.querySelector("#popupOrganisation");
    ...
    table.on("tableBuilt",()=>{
        ...
        colonnes.forEach((col:any) => {
            const def = col.getDefinition();
            
            if(!def.title) return; // Ignore la colonne des checkbox
    ...
    if(popupOrganisation){
        ...
        document.addEventListener("click", (event) => {
            if (popupOrganisation && !popupOrganisation.contains(event.target as Node) && event.target !== btnOrganiser) {
                popupOrganisation.style.display = "none";
            }
        });
    }
...
document.addEventListener("DOMContentLoaded", () => {
    if(fntFiltre){
        ...
        btnReinitialiser?.addEventListener("click", () => {
            if (selectNom) selectNom.selectedIndex = 0;
            if (selectPrenom) selectPrenom.selectedIndex = 0;
            if (selectFonction) selectFonction.selectedIndex = 0;

            if (inputDateDeb) inputDateDeb.value = "";
    ...
    document.addEventListener("click", (event) => {
        if (fntFiltre && !fntFiltre.contains(event.target as Node) && event.target !== btnFiltreGlobal) {
            fntFiltre.style.display = "none";
        }
    });
});
...
btnValider?.addEventListener("click",()=>{
    const listeDate:Date[] = recupDate("#dateDeb-global","#dateFin-global"); // Récupération des dates entrées dans le filtre
    ...
    if (dateDebValide && dateFinValide) {
        table.addFilter("derniereMAJ", "dateRange" as any, {
            dateDeb:listeDate[0],
            dateFin:listeDate[1]
        });
    }
...
const afficherResultat:()=>void = ()=>{
    const totalLigne: HTMLElement|null = document.querySelector("#total-resultat");
    if (totalLigne) {
...
table.on("rowSelectionChanged", (data:any) => {
    const compteur:Element|null = document.querySelector('#compteur-selection');
    const suppr:HTMLButtonElement|null = document.querySelector("#supprimer-ligne");
    const divCroix:HTMLDivElement|null = document.querySelector("#icone-croix");
    const result:HTMLDivElement|null = document.querySelector("#total-resultat");
    const footer:HTMLElement|null = document.querySelector('.tabulator .tabulator-footer .tabulator-footer-contents');
    const iconeCroix = document.querySelector("#icone-croix svg");
    const nbLignes:number = data.length;
    ...
    if(compteur && suppr && divCroix && footer && result){
        ...
        const affichageLigneCaseACoche = (valCompteur:string,valSuppr:string,valCroix:string,valResult:string, valFooter:string)=>{
        ...
        suppr?.addEventListener("click", () => {
            ...
            ligneSelectionne.forEach((ligne:any) => {
